export default function ChiffreChoc() {
  return (
    <section className="bg-white py-14 sm:py-20 px-4 sm:px-6">
      <div className="max-w-3xl mx-auto text-center">
        <p className="font-nunito font-extrabold text-xl sm:text-2xl md:text-3xl text-bleu-dark uppercase leading-tight mb-1">
          <span className="text-orange">30-40%</span> des appels du secteur automobile
        </p>
        <p className="font-nunito font-extrabold text-xl sm:text-2xl md:text-3xl text-bleu-dark uppercase leading-tight mb-3">
          sont perdus ou rappel&eacute;s trop tard
        </p>
        <p className="text-gray-500 text-sm sm:text-base italic">
          (&Eacute;tude Leboncoin &mdash; janvier 2025)
        </p>
      </div>
    </section>
  );
}
