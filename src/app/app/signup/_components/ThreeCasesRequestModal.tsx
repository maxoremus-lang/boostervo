...
export default function ThreeCasesRequestModal({ open, onClose, source }: Props) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setStatus("sending");
    try {
      const res = await fetch("/api/cas-clients", {
        method: "POST",
    ...
    >
      ...
      >
        <div className="flex items-start justify-between p-6 pb-2">
          <div>
            <h2 className="text-xl font-nunito font-extrabold text-bleu">Recevoir 3 cas clients</h2>
            <p className="text-sm text-gray-500 mt-1">On vous les envoie par email.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        {status === "success" ? (
          <div className="p-6 pt-4 text-center">
            ...
            <h3 className="text-lg font-bold text-bleu">Votre demande a bien été prise en compte</h3>
            <button
              type="button"
              onClick={onClose}
          ...
          <form onSubmit={onSubmit} className="p-6 pt-4 space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">
                Mobile <span className="text-gray-400 font-normal normal-case">(optionnel)</span>
              </label>
              <input
                type="tel"
                value={mobile}
