...
async function sendCallbackEmail(payload: { firstName: string; mobile: string; source: string }) {
  if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.warn("[callback-request] SMTP non configuré, notification skip");
    return;
  }
  ...
  const htmlBody = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #1B4F9B; padding: 20px 30px; border-radius: 8px 8px 0 0;">
        ...
        <span style="display: block; margin-top: 6px; font-size: 13px; color: rgba(255,255,255,0.8);">Nouvelle demande de rappel</span>
      </div>
      <div style="background: #fff; padding: 28px 30px; border: 1px solid #eee; border-top: none;">
        <h2 style="color: #1B4F9B; font-size: 18px; margin: 0 0 20px 0;">Un prospect souhaite être rappelé</h2>
        ...
        <div style="margin-top: 24px; padding: 16px; background: #f0f7ff; border-radius: 6px; border-left: 4px solid #FF6600;">
          <p style="margin: 0; font-size: 14px; color: #333;">
            <strong>Action :</strong> rappeler ce prospect dans les meilleurs d&eacute;lais.
          </p>
        </div>
      </div>
  ...
  await transporter.sendMail({
    ...
    subject: `Demande de rappel — ${payload.firstName} (${payload.mobile})`,
    html: htmlBody,
  });
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    await sendCallbackEmail({ firstName, mobile, source });
  } catch (err) {
